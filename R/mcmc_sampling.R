...
tfr.mcmc.sampling <- function(mcmc, thin=1, start.iter=2, verbose=FALSE, verbose.iter=10, uncertainty=FALSE) {
  if (!is.null(mcmc$rng.state)) .Random.seed <- mcmc$rng.state
    ...
    # set up uncertainty steps if using second.stage.uncertainty
    if (mcmc$meta$second.stage.uncertainty){
      m.default.3 <- get.tfr3.mcmc(mcmc$meta$first.stage.directory)
      meta <- mcmc$meta
      nr.countries <- meta$nr.countries
      countries.index <- meta$id_phase3
      mcmc$observations <- m.default.3$mcmc.list[[1]]$observations #**** but which chain to call????
      mcmc$length_obs <- m.default.3$mcmc.list[[1]]$length_obs
      mcmc$recompute.mu.integral <- m.default.3$mcmc.list[[1]]$recompute.mu.integral
      mcmc$recompute.rho.integral <- m.default.3$mcmc.list[[1]]$recompute.rho.integral
    }
